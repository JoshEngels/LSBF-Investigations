...
public:
  KMerStringMinHashFunction(size_t kmerSize, uint32_t key, size_t numHashes, size_t minHashesPerHash)
      : keys(0), kmerSize(kmerSize), minHashesPerHash(minHashesPerHash), numHashes(numHashes) {
    keys.push_back(key);
		...
		for (size_t hashNum = 0; hashNum < numHashes; hashNum++) {
			...
			for (size_t hashPart = 0; hashPart < minHashesPerHash; hashPart++) {
				...
				for (size_t start = 0; start < item.size() - kmerSize + 1; start++) {
					uint64_t hash = getMurmurHash64(item.c_str() + start, kmerSize, keys.at(hashNum * minHashesPerHash + hashPart));
