...
SCENARIO( "Min hash works as expected") {
	...
	GIVEN( "A 3-mer min hash function with multiplicity 2" ) {
		HashFunction<string> *testHashFunction = new KMerStringMinHashFunction(3, 42, 2);
		...
		WHEN( "We hash the same string twice" ) {
				...
				THEN( "The hashes equal each other" ) {
						REQUIRE( hash1 == hash2 );
				}
		}

		WHEN( "When we hash two similar (Jaccard similarity = 0.8) string 100 times with multiplicity 2") {
			...
			THEN( "More than 75%*75% and less than 85%*85% of the hashes equal each other") {
				...
				cout << 0.75 * 0.75 << " " << count / (float)numHashes << " " << 0.85 * 0.85 <<  endl;
			}
		}
	}
}